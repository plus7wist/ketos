...
use crate::function::{first, init, last, tail, Arity, Function, Lambda, SystemFn};
...
use crate::trace::{set_traceback, Trace, TraceItem};
...
impl Machine {
	...
	fn tail_call(&mut self, frame: &mut StackFrame, n_args: u32) -> Result<(), Error> {
		...
		let n = self.stack[start..end].iter().map(|v| v.size()).sum();
	...
	fn clean_stack(&mut self, pos: usize) {
		let n = self.stack[pos..].iter().map(|v| v.size()).sum();
		self.context.set_memory(|m| m.saturating_sub(n));
	...
	fn drain_stack_top(&mut self, n: u32) -> Result<Drain<Value>, ExecError> {
		...
		if n as usize <= len {
			...
			let n = self.stack[start..].iter().map(|v| v.size()).sum();
			self.context.set_memory(|m| m.saturating_sub(n));
	...
	fn get_stack(&self, n: u32) -> Result<&Value, ExecError> {
		self.stack
			.get(n as usize)
			.ok_or_else(|| ExecError::InvalidStack(n))
	}
...
fn get_const(code: &Code, n: u32) -> Result<&Value, ExecError> {
	code.consts
		.get(n as usize)
		.ok_or_else(|| ExecError::InvalidConst(n))
}
